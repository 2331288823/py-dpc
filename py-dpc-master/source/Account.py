...
def input_password():
    password = ""
    if sys.platform.startswith('linux'):
        password = getpass_linux("*")
    elif sys.platform.startswith('win'):
        password = pwd_input_win()
    return password
...
def change_password(password_md5):
    ...
    if password_md5:
        ...
        # 获取旧密码md5值
        old_password_md5 = password_hash(old_password)
        if old_password_md5 == password_md5:
            ...
            while 1:
                ...
                if new_password != "0":
                    # 新密码加密
                    new_password_md5 = password_hash(new_password)
        ...
        print("请输入新密码：", end="")
        new_password = input_password()
        password_md5 = password_hash(new_password)
        return password_md5
